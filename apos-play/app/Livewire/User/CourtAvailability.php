...
class CourtAvailability extends Component
{
    ...
    public function loadAvailability(int $minContinuousHours = 0) // Cambiado a 0 para mostrar todas las canchas
    {
        ...
        foreach ($this->courts as $court) {
            ...
            // Generar horas con su estado
            $hoursWithStatus = $this->generateHoursWithStatus($schedules, $court->id);

            // Generar y guardar el array de horas disponibles con fechas
            $this->available_hours_with_dates[$court->id] = $this->generateAvailableHoursWithDates($hoursWithStatus, $court->id);
        ...
        foreach ($schedules as $schedule) {
            ...
            if ($schedule->is_available == 1) { // 0 = disponible, 1 = no disponible
                ...
                while ($currentHour < $endHourCarbon) {
                    $hourStr = $currentHour->format('H:i');
                    $uniqueKey = $dayOfWeek . '_' . $hourStr;

                    // Agregar la hora al array si no existe
                    if (!isset($hoursArray[$uniqueKey])) {
                        $hoursArray[$uniqueKey] = [
                            'hour' => $hourStr,
    ...
    /**
     * Genera un array con las horas y su estado (disponible/reservado)
     * @param mixed $schedules Colección de horarios
     * @param int $courtId ID de la cancha
     * @return array Array de horas con estado
     */
    private function generateHoursWithStatus($schedules, int $courtId)
    {
        $hoursWithStatus = [];
        $tomorrow = Carbon::tomorrow();
        ...
        // Generar el array de todas las horas posibles según los horarios
        $allHours = $this->generateHoursArray($schedules, $courtId);

        // Determinar el estado de cada hora
        foreach ($allHours as $hourKey => $hourData) {
            $isReserved = $this->isHourReserved($hourKey, $hourData['day_of_week'], $courtId, $reservations);

            $hourData['status'] = $isReserved ? 'reserved' : 'available';
            $hoursWithStatus[$hourKey] = $hourData;
        }

        return $hoursWithStatus;
    }

    /**
     * Genera un array de horas disponibles con sus fechas correspondientes para los próximos 7 días
     * @param array $hoursWithStatus Array de horas con estado
     * @param int $courtId ID de la cancha
     * @return array Array de horas disponibles con fechas
     */
    private function generateAvailableHoursWithDates(array $hoursWithStatus, int $courtId)
    {
        ...
        $hoursByDay = [];
        foreach ($hoursWithStatus as $hourKey => $hourData) {
                ...
                foreach ($hours as $hourKey => $hourData) {
                    try {
                        ...
                        if ($hourData['day_of_week'] == $targetDayOfWeek) {
                            $availableHoursWithDates[] = [
                                'court_id' => $courtId,
                                'hour' => $hourData['hour'], // Usar el dato limpio, no la llave compuesta
                                'date' => $dateStr,
                                'day_of_week' => $targetDayOfWeek,
                                'day_name' => $this->getDayName($targetDayOfWeek),
                                'turn' => $hourData['turn'] ?? 'morning',
                                'schedule_id' => $hourData['schedule_id'] ?? 0,
                                'status' => $hourData['status'] ?? 'available'
                            ];
    ...
    private function isHourReserved(string $hour, int $dayOfWeek, int $courtId, array $reservations)
    {
        ...
        // Asegurarse de limpiar cualquier prefijo que pueda haber quedado
        $cleanHour = $hour;
        if (strpos($hour, '_') !== false) {
            $parts = explode('_', $hour);
            $cleanHour = end($parts);
        }
        $hourFormatted = $cleanHour . ':00';
        ...
        foreach ($reservations as $reservation) {
            ...
            if (
                $hourToCheckFormatted >= $reservationStartFormatted &&
